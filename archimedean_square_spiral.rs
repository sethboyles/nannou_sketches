...
fn view(app: &App, model: &Model, frame: &Frame) {
    ...
    let mut radius = 0.0;
    let mut theta = 0.0;
    let mut current_square_root = 1;
    let mut current_step = 0;

    for number in range {
        
      let x = radius * theta.cos() * 0.0015;
      let y = radius * theta.sin() * 0.0015;

      if is_prime(number){// number % model.num  == 0 { //is_prime(number){
        draw.ellipse().color(WHITE).w(2.0).h(2.0).x_y(x as f32, y as f32);
      }
      

      let steps = (current_square_root + 1).pow(2) - current_square_root.pow(2);
      let increment  = current_step as f32 / steps as f32 * 2.0  * PI;
